#include <iostream>
#include <map>
#include <string>
#include <cmath>
#include <algorithm>
using namespace std;
//utility functions

//TODO: finish binaryToHex function and find the min length of the key
//      (i.e the length of the longest cipher most likelt c7)

int hexToDex (char hex) {
    map<char, int> hexMap = {
        {'a', 10}, {'b', 11}, {'c', 12}, {'d', 13}, {'e', 14}, {'f', 15}
        };
     int dex = (int)hex - 48 > 9 ? hexMap[hex] : (int)hex - 48;
     
     return dex;
}

char dexToHex (int dex) {
    map<int, char> hexMap = {
        {10, 'a'}, {11, 'b'}, {12, 'c'}, {13, 'd'}, {14, 'e'}, {15, 'f'}
    };
    
    char hex = dex > 9 ? hexMap[dex] : char(dex + 48);
    
    return hex;
}
string hexToBinary (char hex) {
    int dex = hexToDex(hex);
    
    string binary = "0000";
    unsigned int counter = 0;
    
    for (int i = 3; i >= 0; i--) {
        if(pow(2, i) <= dex) {
            binary.replace(counter, 1, "1");
            dex -= pow(2, i);
        }
        
        counter++;
    }
    
    return binary;
}

char binaryToHex (string binary) {
    
    unsigned int counter = 0;
    unsigned int sum = 0;
    for (int i = 3; i >= 0; i--) {
        sum += (pow(2, i) * ((int)binary.at(counter) - 48));
        counter++;
    }
    
    return dexToHex(sum);
    
}

char exclusiveOr(char c1, char c2) {
    string b1 = hexToBinary(c1);
    string b2 = hexToBinary(c2);
    
    string result = "0000";
    
    for( unsigned int i = 0; i < b1.length(); i++) {
        if(b1.at(i) != b2.at(i)) {
            result.replace(i, 1, "1");
        }
    }
    
    return binaryToHex(result);
}

string messageXOR (string c1, string c2) {
    
    string result = "";
    for (unsigned int i = 0; i < min(c1.length(), c2.length()); i++) {
        result += exclusiveOr(c1.at(i), c2.at(i));
    }
    
    return result;
}

void compare (string c1, string c2) {
    for (unsigned int i = 0; i < min(c1.length(), c2.length()); i++) {
        if(c1.at(i) == c2.at(i)) {
            cout << "index: " << i << "\nhex: " << c1.at(i) << "\n";
        }
    }
}

void detectSpaces (string m) {
    
     for (unsigned int i = 0; i < m.length(); i += 2) {
         
         if ((int)m.at(i) - 48 >= 4 && (int)m.at(i) - 48 <= 7) {
             cout << "index: " << i << "\nhex: " << m.at(i) << " " << m.at(i + 1) << "\n" ; 
         }
     }
    
    
}


int main() {
   
  string key = "";
  
  string c1 = "";
  
  string c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f";
  
  string c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb";
  
  string c4 = "";
  
  string c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070";
  
  string c6 = "";
  
  string c7 = "";
  
  string c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3";
  
  string c9 = "";
  
  string c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83";
  
  string target_cipher = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904";
  string solution = "54686520736563756574206d65737361676520697320205768656e207573696e6720612073747265616d206369706865722020";
  
 

  
   for (unsigned int i = 0; i < c7.length(); i++) {
      key += "0";
  }
  
detectSpaces(messageXOR(c1, c2);
  
  // 2 ^ 4 = 6 (space only)
  // 2 ^ 5 = 
  // 4 ^ 5 = 1 
  // 4 ^ 4 = 0 
  // 4 ^ 6 = 2
  // 5 ^ 6 = 3
  // 4 ^ 7 = 3
  // 5 ^ 7 = 2
  // 6 ^ 7 = 1
  // 6 ^ 6 = 0
  // 7 ^ 7 = 0
  // 2 ^ 5 = 7 
  // 2 ^ 7 = 5
  // 2 ^ 6 = 4

    
    
    

    return 0;
}
